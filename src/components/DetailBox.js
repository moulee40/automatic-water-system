...
s
  render() {
    ...
    return (
        <div className={`flex flex-col w-64 h-28 border-2 border-solid border-gray-200 rounded relative m-4 shadow-lg ${marginTopLg ? 'mt-28' :''} ${marginRightLg ? 'mr-14' :''}`}>
          <div className='flex justify-center items-center flex-1 border-b-2 border-solid border-gray-400'>{title}(m3/h)</div>
          <div className='flex flex-1'>
            <div className='flex justify-center items-center flex-1 text-center border-r-2 border-solid border-gray-400'><span className='text-sm'>PV Value : </span>{pvValue}</div>
            <div className='flex justify-center items-center flex-1 text-center'><span className='text-sm'>OV Value : </span>{ovValue}</div>
          </div>
        {rightArrow && <>
